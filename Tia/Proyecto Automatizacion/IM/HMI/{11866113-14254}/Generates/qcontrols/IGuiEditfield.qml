...
Rectangle
{
    ...
    TextInput
    {
        ...
        font.pointSize: qm_EditFieldFontSize
        passwordCharacter: "*"
        ...
        Keys.onPressed:
        {
            ...
            utilProxy.keyHandler(qm_EditFieldObjid, event.key, true, event.text,isCtrlPressed, isAltPressed);
            event.accepted = true;
        }

        Keys.onReleased:
        {
            utilProxy.keyHandler(qm_EditFieldObjid, event.key, false, event.text);
            event.accepted = true;
        }
    }
